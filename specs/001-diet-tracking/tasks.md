...
- [ ] T035 [P] [US1] Write `OpenMind.Healthcare/backend/DietApi.Tests/Domain/WeightRecordingTests.cs` proving a second reading on the same date replaces the first rather than adding a second, that `CurrentWeightKg` returns the most recent reading at or before `asOf` and is never null, and that removing the plan's only remaining reading is refused (FR-046)
...
- [ ] T060 [US2] Configure `LoggedDay` and its owned `FoodEntries` in `OpenMind.Healthcare/backend/DietApi/Infrastructure/Data/DietDbContext.cs` `OnModelCreating` with a unique index on `(DietPlanId, Date)`, a supporting index on `(UserId, Date)` for range reads, calorie columns as `int`, macro columns with `HasPrecision`, and `Version` marked `.IsConcurrencyToken()` so EF Core puts the previous value in each `UPDATE`'s `WHERE` clause, and add the `LoggedDays` DbSet
...
- [ ] T074 [US2] Create `OpenMind.Healthcare/backend/DietApi/Features/FoodLog/FoodLogEndpoints.cs` on `/api/food-log` with the four routes from the contract, translating `DbUpdateConcurrencyException` into **409** with a reload message and rejecting an out-of-plan date on the single-day route with **400** (FR-045, FR-032)
